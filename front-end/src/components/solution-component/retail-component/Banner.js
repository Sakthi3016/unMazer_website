...
import main from "../../../assets/images/solution-assests/rti-assests/banner.svg";
          ...
          {mainBannerData.map((item, i) => {
            return (
              <div>
                ...
                <div style={{fontWeight: "450"}}>{item.title}</div>
                <div>{item.subtitle}</div>
                <div className="try_us_button1"><Link to="/demo"><button style={{borderRadius:"4px"}} className="demo-but">Give it a try!<span className="try_us_buttn"></span></button></Link></div>
                </div>
              </div>
            );
          })}
